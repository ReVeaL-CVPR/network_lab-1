...
void BasicClient::run_timer(Timer *timer) {
    if(timer == &_timerTO){
        ...
        WritablePacket *packet = GraphBuilder::_wrapper(buff, DATA, 0, _my_address, _other_address,  strlen(buff));
...
void BasicClient::push(int srcprt, Packet *p) {
    ...
    if(header->type == HELLO){ //data -> send ack
        ...
        WritablePacket *packet = GraphBuilder::_wrapper("", ACK, 0, _my_address, header->source, 1);
        output(0).push(packet);
    } 
    ...
    { //received ack -> schedule new data packet
        ...
        WritablePacket *packet = GraphBuilder::_wrapper("", TCP_ACK, 0, _my_address, header->source, 1);
        output(0).push(packet);
    } 
...
//                      cpEnd) < 0)
//         return -1;
//     WritablePacket *packet = GraphBuilder::_wrapper(buff, TCP_DATA, 0, _my_address, to, strlen(buff));
//     output(0).push(packet);
//     _timerTO.schedule_after_sec(_time_out);
//     return 0;
// }
